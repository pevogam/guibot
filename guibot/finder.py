...
class Finder(LocalConfig):
    ...
    def find(self, needle: "Target | list[Target]", haystack: "Image") -> "list[Match]":
        """
...
class ContourFinder(Finder):
    ...
    def configure(
        ...
        **kwargs: dict[str, type]
        ...
        :paran reset: whether to (re)set all parent configurations as well
        """
...
class TextFinder(ContourFinder):
    ...
    def _detect_text_east(self, haystack: "Image") -> list[tuple[int, int, int, int]]:
        ...
        while True:
            ...
            if len(region_queue) < 2:
                break
            rtuple = region_queue.pop(0)
            r1: tuple[int, int, int, int] = rtuple[0]
            flag1 = rtuple[1]
            if not flag1:
                continue
    ...
    def _detect_rectangle_shapes(self, image: "Array") -> list[dict[str, object]]:
        import cv2
        import numpy as np
        ...
        lines = cv2.HoughLinesP(
            edges, 1, np.pi / 180, threshold=25, minLineLength=1, maxLineGap=15
        )
        ...
        contours, _ = cv2.findContours(
            eroded_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_TC89_L1
        )
